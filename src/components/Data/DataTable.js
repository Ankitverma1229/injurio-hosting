    ...
    <div>
        ...
        <Table className="rounded-md">
          ...
          {injuredData.map((items, i) => (
            <Tbody className="even:bg-gray-200" key={items._id}>
              <Tr className="text-[#092C4C] rounded-md cursor-pointer">
      ...
      {showData ? (
        <div className="fixed z-10 top-0 w-[100vw] h-[100vh] flex justify-center items-center bg-slate-500 bg-opacity-50">
          <div className="w-[90%] md:w-[80%] lg:w-[50%] rounded-md md:p-3 bg-white flex flex-col gap-5 relative">
            <h2 className="text-center text-xl md:text-3xl pt-2 underline">
              Person Details
            </h2>
            <Table>
              <Tbody className="flex flex-col md:gap-3 p-0">
                <Tr className="flex  items-center md:gap-8">
                  <Th className="md:text-2xl fw-bold md:w-1/2 text-center">
                    Name:
                  </Th>
                  <p className="style-text  md:text-xl bg-gray-300 text-center md:w-full rounded-md md:px-5 py-1">
                    {injuriedDetails.name}
                  </p>
                </Tr>
                <Tr className="flex  items-center md:gap-8">
                  <Th className="md:text-2xl fw-bold md:w-1/2">Date:</Th>
                  <p className="style-text md:text-xl bg-gray-300 text-center md:w-full rounded-md md:px-5 py-1">
                    {injuriedDetails.date}
                  </p>
                </Tr>
                <Tr className="flex  items-center md:gap-8">
                  <Th className="md:text-2xl fw-bold md:w-1/2">Time:</Th>
                  <p className="style-text md:text-xl bg-gray-300 text-center md:w-full rounded-md md:px-5 py-1">
                    {injuriedDetails.time}
                  </p>
                </Tr>
                <Tr className="flex  items-center md:gap-8">
                  <Th className="md:text-2xl fw-bold md:w-1/2">Body Parts:</Th>
                  <p className="style-text md:text-xl bg-gray-300 text-center md:w-full rounded-md md:px-5 py-1 overflow-scroll">
                    {injuriedDetails.bodyParts.join(", ")}
                  </p>
                </Tr>
                <Tr className="flex  items-center md:gap-8">
                  <Th className="md:text-2xl fw-bold md:w-1/2">Description:</Th>
                  <p className="style-text md:text-xl bg-gray-300 text-center p-0 md:w-full mx-auto rounded-md md:px-5 py-1 h-20 md:h-32 overflow-scroll">
                    {injuriedDetails.description}
                  </p>
                </Tr>
              </Tbody>
            </Table>
